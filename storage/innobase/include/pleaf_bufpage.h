...
#define PLEAF_VERSION_DATA_SIZE (3 * (1U << 3))
...
#define ASSERT_ARR_IDX(array_index) \
  assert(((0 <= array_index) && (array_index <= 40)))
...
static_assert(PLEAF_PAGE_HEADER_SIZE == sizeof(PLeafPageHeaderData),
        "PLeaf-page header size");

typedef PLeafPageHeaderData* PLeafPageHeader;

#define N_VERSION_DATA_ARR 160 
...
static_assert(PLEAF_PAGE_SIZE == sizeof(PLeafPageData),
        "PLeaf-page size");
...
#define N_PAGE_CAP_ARR 6
#define PLEAF_MAX_CAPACITY 160
...
#define PLeafGetXmin(version) \
  (version->xmin)

#define PLeafGetXmax(version) \
  (version->xmax)
