...
/** @file pleaf/pleaf_bufpage.cc 
 P-Leaf Buffer Page Structure
...
int CAP[N_PAGE_CAP_ARR] = {4, 8, 16, 32, 80, 160};
int CONT_CAP[N_PAGE_CAP_ARR] = {2, 6, 12, 24, 56, INT32_MAX};
int BIT_CAP[N_PAGE_CAP_ARR] = {40, 20, 10, 5, 2, 1};
/*
            ...
            PLeafOffset* offset) 
{
  ...
  ut_a(pos != 0);
  ut_a(pos <= nums + 1);
  ut_a((*bitmap & ((uint64_t)(1) << (pos - 1))) != 0);

  //ret_bitmap = __sync_and_and_fetch(bitmap, ~((uint64_t)(1) << (pos - 1)));
  ret_bitmap = __sync_fetch_and_and(bitmap, ~((uint64_t)(1) << (pos - 1)));
  ut_a((ret_bitmap & ((uint64_t)(1) << (pos - 1))) != 0);
  ret_bitmap &= ~((uint64_t)(1) << (pos - 1));
...
PLeafPageUnsetBitmap(PLeafPage page, int array_index) 
{
  ...
  ut_a((*bitmap & ((uint64_t)(1) << array_index)) == 0);

  ret_bitmap = __sync_fetch_and_or(bitmap, ((uint64_t)(1) << array_index));
  ut_a((ret_bitmap & ((uint64_t)(1) << array_index)) == 0);
